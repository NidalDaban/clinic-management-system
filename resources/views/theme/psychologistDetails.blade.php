...
@section('title', 'Psychologist Details')
            ...
            <div class="container">
                <div class="card">
                    <div class="card-body pt-3">
                        ...
                        <p>{{ $psychologist->job_title }}</p>

                        {{-- Display Average Rating --}}
                        <div class="rating mb-2">
                            @php $stars = floor($psychologist->psychologist_rating ?? 0); @endphp
                            @for ($i = 1; $i <= 5; $i++)
                                @if ($i <= $stars)
                            ...
                            <span class="text-muted ms-1">
                                ({{ number_format($psychologist->psychologist_rating ?? 0, 1) }})
                        ...
                        @forelse($psychologist->psychologistReviews as $review)
                        ...
                        @auth
                            @if (auth()->user()->isPatient())
                                <p>Leave a review for the psychologist below.</p>
                        ...
                        @auth
                            @if (auth()->user()->isPatient())
                                ...
                                <form action="{{ route('reviews.store') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="recipient_id" value="{{ $psychologist->id }}">
